...
async fn get_file_range_limit_cases() -> Result<()> {
    let app = TestApp::spawn().await;
    let url = format!(
        "http://files2.vestibule.io:{}/get_file_range_limit_cases",
        app.port
    );
    app.client
        .put(&url)
...
async fn get_dir_404() -> Result<()> {
    ...
    let resp = app
        .client
        .get(format!("http://files1.vestibule.io:{}/404", app.port))
        .send()
        .await?;
    assert_eq!(resp.status(), 404);
    Ok(())
...
async fn get_dir_zip() -> Result<()> {
    ...
    let resp = app
        .client
        .get(format!("http://files1.vestibule.io:{}/dira?zip", app.port))
        .send()
        .await?;
    assert_eq!(resp.status(), 200);
    assert_eq!(
...
#[tokio::test]
async fn get_dir_zip_404() -> Result<()> {
    let app = TestApp::spawn().await;
    let resp = app
        .client
        .get(format!("http://files1.vestibule.io:{}/404?zip", app.port))
        .send()
        .await?;
    assert_eq!(resp.status(), 404);
    Ok(())
...
async fn head_dir_zip() -> Result<()> {
    ...
    let resp = app
        .client
        .head(format!("http://files1.vestibule.io:{}/?zip", app.port))
        .send()
        .await?;
    assert_eq!(resp.status(), 200);
    assert_eq!(
...
async fn head_file_404() -> Result<()> {
    ...
    let resp = app
        .client
        .head(format!("http://files1.vestibule.io:{}/404", app.port))
        .send()
        .await?;
    assert_eq!(resp.status(), 404);
    Ok(())
...
async fn options_dir() -> Result<()> {
    ...
    let resp = app
        ...
        .request(
            ...
            format!("http://files1.vestibule.io:{}", app.port),
        )
        .send()
        .await?;
    assert_eq!(resp.status(), 200);
...
async fn put_file() -> Result<()> {
    let app = TestApp::spawn().await;
    let url = format!("http://files1.vestibule.io:{}/myfile", app.port);
...
async fn put_file_create_dir() -> Result<()> {
    let app = TestApp::spawn().await;
    let url = format!(
        "http://files1.vestibule.io:{}/xyz/my_file_in_dir.txt",
        app.port
    );
    let resp = app.client.put(&url).body(b"abc".to_vec()).send().await?;
    assert_eq!(resp.status(), 201);
...
async fn put_file_conflict_dir() -> Result<()> {
    ...
    let resp = app
        .client
        .put(format!("http://files1.vestibule.io:{}/dira", app.port))
        .body(b"abc".to_vec())
        .send()
        .await?;
    assert_eq!(resp.status(), 403);
...
async fn delete_file() -> Result<()> {
    let app = TestApp::spawn().await;
    let url = format!(
        "http://files1.vestibule.io:{}/xyz/file_to_delete.txt",
        app.port
    );
...
async fn delete_file_404() -> Result<()> {
    ...
    let resp = app
        .client
        .delete(format!("http://files1.vestibule.io:{}/file3", app.port))
        .send()
        .await?;
    assert_eq!(resp.status(), 404);
    Ok(())
...
async fn propfind_404() -> Result<()> {
    let app = TestApp::spawn().await;
    let url = format!("http://files1.vestibule.io:{}/404", app.port);
    let resp = propfind(&app, &url).send().await?;
    assert_eq!(resp.status(), 404);
    Ok(())
}
...
async fn propfind_file() -> Result<()> {
    let app = TestApp::spawn().await;
    let url = format!("http://files1.vestibule.io:{}/dira/file1", app.port);
    let resp = propfind(&app, &url).send().await?;
    assert_eq!(resp.status(), 207);
    let body = resp.text().await?;
...
async fn propfind_file_encrypted() -> Result<()> {
    ...
    let resp = propfind(
        &app,
        &format!("http://files2.vestibule.io:{}/dira", app.port),
    )
    .send()
    .await?;
    let body = resp.text().await?;
...
async fn mkcol_dir() -> Result<()> {
    let app = TestApp::spawn().await;
    let url = format!("http://files1.vestibule.io:{}/newdir", app.port);
...
async fn copy_file() -> Result<()> {
    ...
    let url = format!("http://files1.vestibule.io:{}/dira/file1", app.port);
    let new_url = format!(
        "http://files1.vestibule.io:{}/dira/file1%20(copy)",
        app.port
    );
...
async fn copy_file_404() -> Result<()> {
    let app = TestApp::spawn().await;
    let url = format!("http://files1.vestibule.io:{}/dira/file3", app.port);
    let new_url = format!(
        "http://files1.vestibule.io:{}/dira/file3%20(copy)",
        app.port
    );
    let resp = copy(&app, &url)
        .header("Destination", new_url)
...
async fn move_file() -> Result<()> {
    let app = TestApp::spawn().await;
    let origin_url = format!("http://files1.vestibule.io:{}/dira/file2", app.port);
    let new_url = format!(
        "http://files1.vestibule.io:{}/dira/file2%20(moved)",
        app.port
    );
...
async fn move_file_404() -> Result<()> {
    let app = TestApp::spawn().await;
    let url = format!("http://files1.vestibule.io:{}/file3", app.port);
    let new_url = format!("http://files1.vestibule.io:{}/file3%20(moved)", app.port);
